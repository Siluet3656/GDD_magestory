...
Action RPG, Roguelike с target боёвкой. Ref: Rabbit&Steel (боёвка), The binding of Issac (передвижение), Slay the spireb (карта).

---
## <a id="title2">Синопсис</a>
Могущественный маг был заточен в подземелье. Выбираться предстоит, проходя различные локации, забегами по карте. Карта состоит из нод разного типа - в конце этажа босс.
Мета: анлок заклинаний + очки талантов
Развитие в забеге: эссенции (предметы как в ROR)

Core Loop (30–60 сек):

* Выбор позиции
* Выбор цели
* Реакция на угрозу (перемещение / дэш / контроль / атака)
* Победа над врагом
* Награда → усиление билда
...
Encounter loop (30–60 сек):

* Читается текст с событием
* Выбор варианта из предложенных
* Получение эффектов (положительные / отрицательные / сюжетные)

Battle loop (90–120 сек):

* Генерация карты
* Поиск врагов по карте
* Битва с врагами
* Сбор наград

Boss loop (120–180 сек):

*  Сразу столкновение с боссом без поиска и генерации в комнате
*  Сбор наград
...
1. Система ХП
  ...
  - механика восстановления здоровья: через способности
  - механика оверхп: "щит" в виде дополнительного здоровья поверх основного
  - Механика блок-щита: "щит" который защищает от n атак независимо от их урона
...
5. Таргет система
  ...
  - некоторые заклинания можно размещать через мышь+лкм
6. Диалоги
  ...
  - ещё нпц?
7. Предметы
  - Покупаются у торговца во время забега в "рогаликовой" части и дают билеты: за билеты берутся особые заклинания (арканумы) в лобби.
### Тэги заклинаний
...
- заражение: наносит n урона (при наложении, после раз в секунду). После каждого тика урона имеет шанс, равный шансу  получить крит удар на передачу этого эффекта случайной цели в небольшом радиусе вокруг(кроме целей под этим эффектом). При передаче длительность сокращается вдвое. После окончания эффекта цель получает эффект "иммунитет".
- иммунитет: на цель невозможно наложить эффект "заражение".
...
### Система заклинаний
Для каста необходимо вынести скилл из книги на панель. После нажать соответствующий кейбинд на панели. При касте скорость движения меньше. Отмена каста по кнопке. Арканумы - мощные заклинания с кулдаунами и большей стоимостью. Все заклинания кастуюься из шарда, который плавно летает за играоком.
#### Заклинания
- Огонь:
...
- Арканумы:
  1. Огненный шторм: выпускает залп "огненных шаров" в цель
  2. Метка смерти: цель, не являющаяся боссом, мгновенно погибает, если её здоровье ниже порога в n%
...
#### Battles
  - Зачистка комнат
#### Boss
  - Битва с магом-стражем
### Таланты
  // Здесь надо нарисовать картинку с талантами
### Тэги противников
...
### Босс
Для победы нужно снести всё ХП боссу. полоска ХП одна единственная. Фазы зависят от ХП
#### Атаки
...
- Вынужденная защита: босс быстро спавнит 3 сферы защиты в одной точке.
...
0. База:
  - В лобби выбираем скиллы 
  - В лобби качаем таланты (20 очков)
  - Идём в забег
  - В забеге прогрессия предметами
  - После прохождения/победы даётся опыт для апа уровней
2. Лобби:
  - Статичная книга с заклинаниями - смена закл
...
Персонажи:
  1. Торговец - давний знакомый гг, Кентози, прибыл что бы спа... продавать товары по завышенным ценам.

Прогресс:
  1. Обучение
     - После небольшой сценки с чёрным экраном гг просыпается в камере в цепях (вырывается)
     - Продвигаясь по началу от него бегут стражники (боятся). Некоторые умирают от страха.
